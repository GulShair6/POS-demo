...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      return Response.json({ error: "Valid name, email, role and temporary password are required" }, { status: 400 });
    ...
    await db.insert(auditLogs).values({
      ...
      details: `${employee.name} · ${employee.role}`
    });
    const safeEmployee = {
      id: employee.id,
      name: employee.name,
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!payload.id || (payload.role && !allowedRoles.includes(payload.role)))
      return Response.json({ error: "Valid employee and role are required" }, { status: 400 });
    if (payload.id === auth.user.id && (payload.active === false || (payload.role && payload.role !== auth.user.role)))
      return Response.json({ error: "You cannot disable or change the role of your active account" }, { status: 409 });
    const db = await getDb();
    const [employee] = await db
      .update(employees)
      .set({
    ...
    await db.insert(auditLogs).values({
      ...
      entityType: "employee",
      entityId: String(employee.id),
      details: `${employee.name} · ${employee.role} · ${employee.active ? "active" : "inactive"}`
    });
    const safeEmployee = {
      id: employee.id,
      name: employee.name,
