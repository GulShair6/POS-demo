...
export function calculateSaleTotals(lines: Array<{ unitPrice: number; quantity: number }>, taxRate: number) {
  ...
  const subtotal = roundMoney(lines.reduce((sum, line) => {
    if (!Number.isFinite(line.unitPrice) || line.unitPrice < 0 || !Number.isInteger(line.quantity) || line.quantity < 1) throw new Error("Invalid sale line");
...
export function resolveSplitTender(total: number, requestedCash?: number, requestedCard?: number) {
  ...
  if (cash < 0 || card < 0 || Math.abs(cash + card - total) > 0.01) throw new Error("Split payments must equal the sale total");
  return { cash, card };
}

export function calculateRefundLine(unitPrice: number, quantity: number, originalSubtotal: number, originalTax: number) {
