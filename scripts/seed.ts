...
if (process.env.SEED_DEMO_DATA === "true") {
  ...
  if (!hasProducts) await db.insert(products).values([
  ...
  if (!hasCustomers) await db.insert(customers).values([{ name:"Maya Patel", email:"maya@example.com" },{ name:"Noah Williams", email:"noah@example.com" }]);
}
