...
test("deployment example contains required security and database values", async () => {
  ...
  for (const key of ["DATABASE_URL","POSTGRES_PASSWORD","ADMIN_EMAIL","ADMIN_PASSWORD","SESSION_SECRET","POS_TAX_RATE"]) assert.match(text, new RegExp(`^${key}=`, "m"));
});

test("compose keeps PostgreSQL private and binds the app to loopback", async () => {
  const text = await (await import("node:fs/promises")).readFile(new URL("../docker-compose.yml", import.meta.url), "utf8");
