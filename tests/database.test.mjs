...
test("migration and an atomic sale ledger execute on PostgreSQL", async () => {
  ...
  await db.transaction(async (tx) => {
    await tx.exec(
      `INSERT INTO sales(receipt_number,idempotency_key,shift_id,employee_name,subtotal,tax,total) VALUES ('AT-1','idem-1',1,'Owner',4.50,0.37,4.87)`
    );
    await tx.exec(
      `INSERT INTO sale_lines(sale_id,product_id,product_name,sku,unit_price,unit_cost,quantity,line_total) VALUES (1,1,'Coffee','COF-1',4.50,1.00,1,4.50)`
    );
    await tx.exec(`INSERT INTO payments(sale_id,method,amount,tendered) VALUES (1,'Cash',4.87,5.00)`);
    await tx.exec(
      `INSERT INTO inventory_movements(product_id,sale_id,type,quantity,reason) VALUES (1,1,'sale',-1,'AT-1')`
    );
  ...
  const result = await db.query(
    `SELECT p.stock, s.total, r.expected_cash FROM products p CROSS JOIN sales s CROSS JOIN register_shifts r`
  );
