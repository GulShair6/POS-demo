...
export const registerShifts = pgTable(
  "register_shifts",
  {
    ...
    openedAt: time("opened_at").notNull().defaultNow(),
...
export const sales = pgTable(
  "sales",
  {
    ...
    shiftId: integer("shift_id").references(() => registerShifts.id),
    employeeId: integer("employee_id").references(() => employees.id),
    employeeName: text("employee_name").notNull(),
  ...
  (table) => [
    uniqueIndex("sales_receipt_uidx").on(table.receiptNumber),
    uniqueIndex("sales_idempotency_uidx").on(table.idempotencyKey),
